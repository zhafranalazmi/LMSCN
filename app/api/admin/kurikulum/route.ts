...
export async function GET() {
  ...
  await connectDB();
  const list = await Kurikulum.find().select("-password").sort({ name: 1 });
...
export async function POST(req: Request) {
  ...
  const existing = await Kurikulum.findOne({ email: email.toLowerCase() });
  if (existing) {
  ...
  const akun = await Kurikulum.create({
    name,
